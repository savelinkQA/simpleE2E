...
UPDATE
Добавлена функция записи шагов из действий в браузере (см ниже Запись шагов в новый `.feature` (UI-рекордер))
...
## Запись шагов в новый `.feature` (UI-рекордер)

Чтобы записывать клики/ввод/переходы и получить готовый `.feature` файл:

1) Запускай в видимом браузере и включай запись:

```
behave -D HEADLESS=0 -D RECORD=1
```

2) В сценарии добавь шаг:
...
3) Записанный файл будет сохранён в `recorded_features/`
